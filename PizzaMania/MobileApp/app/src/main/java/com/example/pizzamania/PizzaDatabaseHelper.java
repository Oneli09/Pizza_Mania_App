...
public class PizzaDatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final int DATABASE_VERSION = 2; // 👈 INCREASED TO TRIGGER onUpgrade()
    ...
    public void onCreate(SQLiteDatabase db) {
        // Create Menu Table with 'category' column
    ...
    private List<CartItem> getOrderItems(SQLiteDatabase db, int orderId) {
        ...
        while (itemCursor.moveToNext()) {
            ...
            if (pizzaCursor.moveToFirst()) {
                ...
                String category = pizzaCursor.getString(pizzaCursor.getColumnIndexOrThrow("category")); // 👈 ADDED

                MenuModel pizza = new MenuModel(pizzaId, name, description, price, imageUrl, branch, stock, category); // 👈 UPDATED
    ...
    // 👇 NEW METHOD: Get items by category
    public List<MenuModel> getMenuItemsByCategory(String category) {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                menuList.add(new MenuModel(id, name, description, price, imageUrl, branch, stock, category));
